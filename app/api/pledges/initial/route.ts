...
const inputSchema = z.object({ amountCents: z.number().int().nonnegative().max(100_000_000) }).strict();
...
export async function POST(request: Request) {
  try {
    ...
    return dataResponse(await new PledgeService(getDb()).recordInitialPledge(user.id, input.amountCents, key), { status: 201 });
