...
export async function POST(request: Request, context: { params: Promise<{ chiliId: string }> }) {
  try { const admin = await requireAdminRequest(request); const id = z.coerce.number().int().positive().parse((await context.params).chiliId); return dataResponse(await new AdminChiliService(getDb()).deactivateChili(admin.id, id, schema.parse(await request.json()).reason)); }
